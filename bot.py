...
MINI_APP_URL = os.getenv("MINI_APP_URL", "https://sherrzodbe-star.github.io/hotdog-miniapp/")
...
@app_web.post("/api/order")
def create_order():
    data = request.get_json(silent=True) or {}
    if not validate_init_data(data.get("initData", "")):
        return jsonify(ok=False, error="Telegram sessiyasi tasdiqlanmadi"), 401
    ...
    if not name or not phone or not isinstance(raw_items, list) or not raw_items:
        return jsonify(ok=False, error="Buyurtma ma'lumotlari to'liq emas"), 400

    lines = ["🆕 YANGI BUYURTMA (Mini App)", f"👤 Mijoz: {name}", f"📞 Tel: {phone}", f"💳 To'lov: {payment or '-'}", "", "🛒 Buyurtma:"]
    total = 0
    for row in raw_items:
        item_id = str(row.get("id", ""))
        if item_id not in MENU:
            return jsonify(ok=False, error="Noma'lum mahsulot"), 400
        try:
            qty = int(row.get("qty", 0))
        except Exception:
            return jsonify(ok=False, error="Miqdor xato"), 400
        if qty < 1 or qty > 99:
            return jsonify(ok=False, error="Miqdor noto'g'ri"), 400
        title, unit, emoji = MENU[item_id]
        subtotal = unit * qty
        total += subtotal
        lines.append(f"• {emoji} {title} x{qty} = {price(subtotal)}")
    lines.append(f"\n💰 Jami: {price(total)}")
    if note:
    ...
    if errors:
        return jsonify(ok=False, error="Telegramga yuborishda xatolik"), 502
    return jsonify(ok=True)
...
async def address(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer()
    await q.message.reply_text("📍 Termiz shahri, Cola restoran ro‘parasi\n📞 +998 95 156 82 88 / +998 95 147 83 88")

async def web_app_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Only used if the Mini App is later launched from a reply-keyboard web_app button.
    try:
        order = json.loads(update.effective_message.web_app_data.data)
        await update.effective_message.reply_text("✅ Buyurtmangiz qabul qilindi!")
    except Exception:
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(address, pattern="^address$"))
